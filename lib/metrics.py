...
class Meter(object):
    ...
    def compute(self, eps=np.finfo(np.float64).eps):
        ...
        self._count()
        self._op = sum(self._tp_num) / sum(self._pd_num)
        self._or = sum(self._tp_num) / sum(self._gt_num)
...
class TopkMeter(Meter):
    def __init__(self, num_classes, ignore_path=None, threshold=0.5, topk=3):
